# 技术实现方案（SearchEvery 桌面端）

## 1. 概述
SearchEvery 是一款跨平台桌面应用（Windows/macOS/Linux），用于全盘文件检索与管理。系统通过倒排索引 + 向量检索混合搜索实现高性能、高召回的检索体验，并提供重复文件识别与清理功能。支持中文分词（jieba），可选接入本地/云端大模型用于图片/音频/视频的识别增强，及 Milvus/ChromaDB/LanceDB 等向量数据库。
